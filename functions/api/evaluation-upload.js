// Cloudflare Pages Function — FAF Evaluation Video Upload
// Private upload endpoint for player evaluation videos (tryout footage).
// Uploads land in R2 only — coaches review before any decisions are made.
//
// Security gates:
...
async function checkRateLimit(kvStore, clientIp) {
  ...
  const key = `ratelimit:eval-upload:${clientIp}`;
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!passcode) {
      return new Response(
        JSON.stringify({ error: "Evaluation upload is not configured." }),
        { status: 500, headers },
      );
    }
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message:
          "Upload received. Thank you! Coaches will review this evaluation video.",
      }),
      { status: 200, headers },
    );
  } catch (err) {
